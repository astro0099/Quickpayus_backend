...
export const createFeedback = catchAsyncErrors(
  async (req: any, res: Response, next: NextFunction) => {
    ...
    if (!priority) {
      res.send({
        success: false,
        message: 'Priority must be defined',
      });
      return;
    }
...
export const getTicketOrFeedbackBySearch = catchAsyncErrors(
  async (req: any, res: Response, next: NextFunction) => {
    ...
    ticketQuery.userId = id;
    ticketQuery.subject = { $regex: searchText, $options: 'i' };

    const tickets = await Ticket.find(ticketQuery);

    feedbackQuery.userId = id;
    feedbackQuery.fbCnt =  { $regex: searchText, $options: 'i' };
